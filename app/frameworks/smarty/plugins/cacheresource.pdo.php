...
class Smarty_CacheResource_Pdo extends Smarty_CacheResource_Custom
{
    ...
    protected function fillStatementsWithTableName ()
    {
        ...
        $this->insertStatement   = sprintf ( $this->insertStatement,
                                             $this->getTableName () );
        $this->deleteStatement   = sprintf ( $this->deleteStatement,
                                             $this->getTableName (), '%s' );
        $this->truncateStatement = sprintf ( $this->truncateStatement,
                                             $this->getTableName () );
    ...
    protected function getFetchStatement ( $columns, $id, $cache_id = null,
                                           $compile_id = null )
    {
     ...
     * @access protected
     */
    protected function fetch ( $id, $name, $cache_id = null, $compile_id = null,
                               &$content, &$mtime )
    {

        $stmt = $this->getFetchStatement ( $this->fetchColumns, $id, $cache_id,
                                           $compile_id );
        $stmt->execute ();
        $row  = $stmt->fetch ();
        $stmt->closeCursor ();
     ...
     * @access protected
     */
    protected function save ( $id, $name, $cache_id = null, $compile_id = null,
                              $exp_time, $content )
    {
        ...
        $stmt->bindValue ( 'cache_id', $cache_id,
                           (is_null ( $cache_id )) ? PDO::PARAM_NULL : PDO::PARAM_STR  );
        $stmt->bindValue ( 'compile_id', $compile_id,
                           (is_null ( $compile_id )) ? PDO::PARAM_NULL : PDO::PARAM_STR  );
     ...
     * @access protected
     */
    protected function delete ( $name = null, $cache_id = null,
                                $compile_id = null, $exp_time = null )
    {

        // delete the whole cache 
        if ( $name === null && $cache_id === null && $compile_id === null && $exp_time
                === null )
        {
        ...
        // run delete query 
        $query = $this->pdo->query ( sprintf ( $this->deleteStatement,
                                               join ( ' AND ', $where ) ) );
